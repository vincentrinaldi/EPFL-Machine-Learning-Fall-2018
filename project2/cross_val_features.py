...
def cross_val_features():
    ...
    for indx, k_val in enumerate(ks):
        rmse_train_each_fold = []
        rmse_test_each_fold = []
    
        print("For num_features :", k_val)
        ...
        for k in range(k_fold):
            ...
            user_features, item_features, nz_train, nz_test = init_latent_vectors(num_users, num_items, k_val, train, test)
    ...
    plt.semilogx(ks, rmse_tr, marker=".", color='b', label='train error')
    plt.semilogx(ks, rmse_te, marker=".", color='r', label='test error')
    plt.xlabel("Number of Features")
    plt.ylabel("RMSE")
    plt.title("Effect of Number of Features")
    plt.legend(loc=2)
    plt.grid(True)
    plt.savefig("Effect_num_features")
