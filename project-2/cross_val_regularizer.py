...
"""Cross-validation to find optimal regularizer term"""
	
	
def cross_val_regularizer():
    ...
    for indx, regularizer_val in enumerate(regularizers):
        rmse_train_each_fold = []
        rmse_test_each_fold = []
    
        print("For regularizer :", regularizer_val)
        ...
        for k in range(k_fold):
            ...
            for it in range(max_epochs):
                ...
                for d, n in nz_train:
                    ...
                    item_features[n, :] += lrate * (err * user_info - regularizer_val * item_info)
                    user_features[d, :] += lrate * (err * item_info - regularizer_val * user_info) 
    ...
    plt.semilogx(regularizers, rmse_tr, marker=".", color='b', label='train error')
    plt.semilogx(regularizers, rmse_te, marker=".", color='r', label='test error')
    plt.xlabel("Regularization Term")
    plt.ylabel("RMSE")
    plt.title("Effect of Regularization Term")
    plt.legend(loc=2)
    plt.grid(True)
    plt.savefig("Effect_regularization_term")
